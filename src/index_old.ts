...
import { pickChainAndContract, fetchCoinDetails, CGSearchResultCoin, CGCoinDetails, fetchMarketsByIds, CGMarketRow, fetchCoinsList } from './coingecko';
...
async function main(): Promise<void> {
    ...
    // Счётчики прогресса
    // Простые кеши на время выполнения
    const cgSearchCache = new Map<string, CGSearchResultCoin | null>();
    const cgDetailsCache = new Map<string, CGCoinDetails>();
    const cgMarketsCache = new Map<string, CGMarketRow>();
    ...
    // 1) Предразрешим кандидатов по полному списку CG без множества search-запросов
    const uniqueBases = Array.from(new Set(perpSymbols.map((s) => parseBaseAsset(s.baseAsset).baseNormalized)));
    console.log(`[info] Уникальных базовых символов: ${uniqueBases.length}`);
    console.log('[info] Загружаю полный список монет CoinGecko...');
    ...
    const baseToCandidate = new Map<string, CGSearchResultCoin | null>();
    const candidateIdsSet = new Set<string>();
    for (const base of uniqueBases) {
        const group = symbolToCoins.get(base.toLowerCase()) || [];
        for (const g of group) candidateIdsSet.add(g.id);
    }
    const candidateIds = Array.from(candidateIdsSet);
    console.log(`[info] Кандидатных id для markets: ${candidateIds.length}`);
    let marketsMap: Record<string, CGMarketRow> = {};
    try {
        marketsMap = await fetchMarketsByIds(candidateIds);
    } catch (e) {
        console.warn('[warn] Не удалось загрузить markets с первого раза, продолжу без них (cg_not_found). Ошибка:', (e as Error)?.message ?? e);
        marketsMap = {};
    }
    for (const [id, row] of Object.entries(marketsMap)) cgMarketsCache.set(id, row);
    // Выберем лучшего по market_cap для каждого base
    for (const base of uniqueBases) {
        const group = symbolToCoins.get(base.toLowerCase()) || [];
        if (group.length === 0) { baseToCandidate.set(base, null); continue; }
        let best: { id: string; symbol: string; name: string } | null = null;
        let bestCap = -1;
        for (const g of group) {
            const m = cgMarketsCache.get(g.id);
            const cap = m?.market_cap ?? -1;
            if (cap != null && cap > bestCap) { bestCap = cap; best = g; }
        }
        if (!best) best = group[0];
        baseToCandidate.set(base, best ? { id: best.id, symbol: best.symbol, name: best.name, market_cap_rank: null } : null);
    }
	...
	await Promise.all(
        perpSymbols.map((sym) =>
            limiter(async () => {
                ...
                // CoinGecko: используем предразрешённого кандидата и рынки; детали для контрактов только при match
                ...
                let cgOk = false;
                try {
                    const best = baseToCandidate.get(baseNormalized) ?? null;
                    if (best) {
                        const m = cgMarketsCache.get(best.id);
                        cgId = best.id;
                        cgSymbol = best.symbol;
                        cgName = best.name;
                        cgPrice = m?.current_price ?? null;
				...
				// Валидация: цена в пределах толеранса
                ...
                // При совпадении цены, подтянем платформы один раз
                if (cgOk && priceOk && cgId) {
                    try {
                        let details = cgDetailsCache.get(cgId);
                        if (!details) {
                            details = await fetchCoinDetails(cgId);
                            cgDetailsCache.set(cgId, details);
                        }
