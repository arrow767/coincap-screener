...
import { pickChainAndContract, pickBestCoinCandidate, searchCoins, fetchCoinDetails, CGSearchResultCoin, CGCoinDetails, initCgClients } from './coingecko';
import { parseBaseAsset, computePriceDiffPct } from './match';
import { loadConfig } from './config';
import { loadProxies, getProxyCount } from './proxy';
	...
	await Promise.all(
        perpSymbols.map((sym) =>
            limiter(async () => {
                ...
                let cgOk = false;
                try {
                    let best = cgSearchCache.get(baseNormalized) ?? null;
                    if (best === undefined) best = null;
                    if (best === null && !cgSearchCache.has(baseNormalized)) {
                        const candidates = await searchCoins(baseNormalized);
                        best = pickBestCoinCandidate(baseNormalized, candidates);
                        cgSearchCache.set(baseNormalized, best ?? null);
                        if (!best && processed < 10) {
                            console.log(`[debug] ${sym.symbol}: search="${baseNormalized}" -> candidates=${candidates.length}, best=null`);
                        }
                    }
					if (best) {
                        let details = cgDetailsCache.get(best.id);
                        if (!details) {
                            details = await fetchCoinDetails(best.id);
                            cgDetailsCache.set(best.id, details);
                        }
